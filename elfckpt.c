 ...
 * beginning of the checkpoint image.
 * ckpt_fini_elfstream completes the executable.
 */
void
ckpt_init_elfstream(int fd, unsigned long ckptsize)
{
	...
	rv = xwrite(e.fd, base+sizeof(Elf32_Ehdr),
		    restartbinlen-sizeof(Elf32_Ehdr));
	if(0 > rv)
		fatal("cannot write checkpoint file\n");
...
void
ckpt_fini_elfstream(int fd)
{
	int rv;
	char zero[PAGE_SIZE];

	if(fd != e.fd)
		fatal("bad elfstream reference\n");
